...
uint8_t HID_Rx_Buffer[16];
uint8_t USB_HID_Connect_Status = 0;
...
__ALIGN_BEGIN static uint8_t USBD_HID_CfgFSDesc[USB_HID_CONFIG_DESC_SIZ] __ALIGN_END =
    {
        ...
        0x00,
        HID_FS_BINTERVAL_IN, /* bInterval: Polling Interval */
        // HID_EPOUT
        0x07,                   /* bLength: Endpoint Descriptor size */
        USB_DESC_TYPE_ENDPOINT, /* bDescriptorType:*/
        ...
        0x00,
        HID_FS_BINTERVAL_OUT /* bInterval: Polling Interval */
        /* 40 */
};
...
static uint8_t USBD_HID_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
    ...
    USBD_LL_PrepareReceive(pdev, HID_EPOUT_ADDR, HID_Rx_Buffer, 16);
...
static uint8_t USBD_HID_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
    ...
    pdev->ep_in[HID_EPOUT_ADDR & 0xFU].is_used = 0U;
    pdev->ep_in[HID_EPOUT_ADDR & 0xFU].bInterval = 0U;
 ...
 * @param                           {*}
 * @return                          {*}
 * @param {USBD_HandleTypeDef} *pdev
 */
uint8_t USBD_HID_Check_Connect(USBD_HandleTypeDef *pdev)
{
    return (uint8_t)(pdev->dev_state == USBD_STATE_CONFIGURED);
}
