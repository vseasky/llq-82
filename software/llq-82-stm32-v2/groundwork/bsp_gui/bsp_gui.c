...
uint8_t mx_oled_gui_task6[MAX_TASK6_NUM][MAX_SIZE_LENGTH] =
	{".left_enc_dir  ", ".right_enc_dir  ", ".default password", ".lcd_touch_power  ", ".rgb_power  ", ".lcd_sleep_time", ".esp32_mac"};
...
{
	//设置键盘输入开启条件
	if ((mx_oled_info_s.oled_directory == 3) && ((mx_oled_info_s.oled_level1_options == 0) | (mx_oled_info_s.oled_level1_options == 5)))
	{
		keyboard_input_mode();
	}
	else
...
static void interface_task6(void)
{
	...
	if (mx_oled_info_s.oled_directory == 3)
	{
		...
		else if ((mx_oled_info_s.oled_level2_options[5] == 6) && ((mx_oled_info_s.oled_key_button_s == mx_key_right) || (mx_oled_info_s.oled_key_button_s == mx_key_on)))
		{
			set_now_mac();
			flash_save_setting();
		}
