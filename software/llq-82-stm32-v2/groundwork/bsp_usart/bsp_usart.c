...
{
    static uint16_t this_time_rx_len1 = 0;
                    ...
                    if ((hdma_usart3_rx.Instance->CR & DMA_SxCR_CT) == RESET)
                        {
                            ...
                            this_time_rx_len1 = USART_RX_LEN - hdma_usart3_rx.Instance->NDTR;
                            //重新设定数据长度
                            hdma_usart3_rx.Instance->NDTR = USART_RX_LEN;
                            //设定缓冲区1
                            ...
                            __HAL_DMA_ENABLE(&hdma_usart3_rx);
                            /*计算缓冲区0*/
                            bsp_bm2166_receive(usart3_rx_buf[0],this_time_rx_len1);
                        }
                    else
                        {
                            //失效DMA
                            ...
                            __HAL_DMA_ENABLE(&hdma_usart3_rx);
                            /*计算缓冲区1*/
                            bsp_bm2166_receive(usart3_rx_buf[1],this_time_rx_len1);
                        }
                }
        }
                    ...
                    if ((hdma_usart1_rx.Instance->CR & DMA_SxCR_CT) == RESET)
                        {
                            ...
                            __HAL_DMA_DISABLE(&hdma_usart1_rx);
                            //获取接收数据长度,长度 = 设定长度 - 剩余长度
                            this_time_rx_len2 = USART_RX_LEN - hdma_usart1_rx.Instance->NDTR;
                            //重新设定数据长度
                            hdma_usart1_rx.Instance->NDTR = USART_RX_LEN;
                            //设定缓冲区1
                            ...
                            __HAL_DMA_ENABLE(&hdma_usart1_rx);
                            /*计算缓冲区0*/
                            ble_rx_buff_updata(usart1_rx_buf[0],this_time_rx_len2);
                        }
                    else
                        {
                            //失效DMA
                            ...
                            __HAL_DMA_ENABLE(&hdma_usart1_rx);
                            /*计算缓冲区1*/
                            ble_rx_buff_updata(usart1_rx_buf[1],this_time_rx_len2);
                        }
                }
        }
}