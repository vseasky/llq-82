...
/*前12字节供键盘正常程序使用,后16字节供esp-now使用*/
...
{
    /*18 字节 正常共享*/
    uint8_t key_press[10];//按键按下状态
    uint8_t enc_press;    //编码器按键按下状态
    uint8_t enc_value[2]; //编码器值
    uint8_t touch_id;     //指纹识别结果
    uint8_t customize[4]; //自定义功能
} llq_now_info;
typedef struct 
{
