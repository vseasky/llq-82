/*
 * @author                          : Seasky.Liu
 * @LastEditTime: 2022-02-27 19:49:07
...
extern enc_key key_enc_info; //编码器按键信息
...
char input_buffer3[MAX_PASS_WORD_LEN] = "";
uint8_t utf8_mac[6] = {0};
...
uint8_t check_hex(char *hex_p)
{
    if ((hex_p[0] >= '0') & (hex_p[0] <= '9'))
    {
        return hex_p[0] - '0';
    }
    else if ((hex_p[0] >= 'a') & (hex_p[0] <= 'f'))
    {
        return hex_p[0] - 'a'+10;
    }
    else if ((hex_p[0] >= 'A') & (hex_p[0] <= 'F'))
...
{
    flash_get_mac(utf8_mac);
    snprintf(input_buffer3, 18, "%02x:%02x:%02x:%02x:%02x:%02x", utf8_mac[0], utf8_mac[1], utf8_mac[2], utf8_mac[3], utf8_mac[4], utf8_mac[5]);
}
void set_now_mac(void)
{
    flash_set_mac(utf8_mac);
}
void check_mac(uint8_t *mac)
{
    /* ff:ff:ff:ff:ff:ff */
    utf8_mac[0] = (check_hex(&mac[0]) << 4) | (check_hex(&mac[1]));
    if (mac[1] != '\0')
    {
        if ((mac[2] != ':') & (mac[2] != '\0'))
        {
    ...
    utf8_mac[1] = (check_hex(&mac[3]) << 4) | (check_hex(&mac[4]));
    if (mac[4] != '\0')
    {
        if ((mac[5] != ':') & (mac[5] != '\0'))
        {
    ...
    utf8_mac[2] = (check_hex(&mac[6]) << 4) | (check_hex(&mac[7]));
    if (mac[7] != '\0')
    {
        if ((mac[8] != ':') & (mac[8] != '\0'))
        {
    ...
    utf8_mac[3] = (check_hex(&mac[9]) << 4) | (check_hex(&mac[10]));
    if (mac[10] != '\0')
    {
        if ((mac[11] != ':') & (mac[11] != '\0'))
        {
    ...
    utf8_mac[4] = (check_hex(&mac[12]) << 4) | (check_hex(&mac[13]));
    if (mac[13] != '\0')
    {
        if ((mac[14] != ':') & (mac[14] != '\0'))
        {
    ...
    utf8_mac[5] = (check_hex(&mac[15]) << 4) | (check_hex(&mac[16]));
    mac[17] = '\0';
}
void keyboard_buffer3_show(uint8_t options)
{
    check_mac(input_buffer3);
    lv_set_gui_input(4, input_buffer3, options, 1);
    key_info.keyboard_user = LLQ_INPUT_BUFFER3;
}
...
static void esp_now_updata(void)
{
    uint8_t i = 0;
    for(i=0;i<10;i++)
    {
        bsp_ble_addr.esp_now_p.key_press[i] = 0;
    }
    for (i = 0; i < 80; i++)
    {
        bsp_ble_addr.esp_now_p.key_press[i/8] |= (key_info.key_is_press[i].press_it.states != KEY_UPSPRING)<<(i%8); 
    }
    bsp_ble_addr.esp_now_p.enc_press = ((key_enc_info.key_t[0].key_info.states)<<4)|(key_enc_info.key_t[1].key_info.states);
    bsp_ble_addr.esp_now_p.enc_value[0] = key_enc_info.encoder_t[0].enc_t;
    bsp_ble_addr.esp_now_p.enc_value[1] = key_enc_info.encoder_t[1].enc_t;
    bsp_ble_addr.esp_now_p.touch_id     = get_touch_page();
    bsp_ble_addr.esp_now_p.customize[0] = 0;
    bsp_ble_addr.esp_now_p.customize[1] = 0;
    bsp_ble_addr.esp_now_p.customize[2] = 0;
    bsp_ble_addr.esp_now_p.customize[3] = 0;
}
...
static void input_char_updata(char *char_t)
{
    ...
    break;
    case LLQ_INPUT_BUFFER3:
    {
        //密码输入长度限制
        if (strlen(input_buffer3) <= MAX_PASS_WORD_LEN)
        {
            strcat(input_buffer3, char_t);
        }
    }
    break;
    }
...
static void delete_one_char(void)
{
    ...
    case LLQ_INPUT_BUFFER3:
    {
        length = strlen(input_buffer3);
        if (length > 0)
        {
            input_buffer3[length - 1] = '\0';
        }
    };
    break;
    }
