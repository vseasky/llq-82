 ...
 * @Author                         : Seasky.Liu
 * @Date                           : Do not edit
 * @LastEditTime: 2022-02-27 16:46:58
...
extern llq_store_ctr falsh_store_ctr;
...
void touch_delect_callback(uint8_t ok_t, uint16_t page_id_t, uint16_t mathscore_t)
{
    if (touch_task_ctr.touch_status == TOUCH_AUTO_DELECT)
    {
        ...
        if (touch_task_ctr.touch_delect.mathscore > 75)
        {
            touch_task_ctr.touch_delect.page_id = page_id_t;
            if (falsh_store_ctr.llq_store_p.touch_point[touch_task_ctr.touch_delect.page_id] == 0)
            {
...
{
    return touch_task_ctr.touch_delect.page_id;
}
...
void touch_show_info(uint8_t level, uint8_t enc_t, uint8_t enable)
{
    ...
    case 1:
    {
        if (enable)
        {
            touch_task_ctr.touch_add_num = enc_t % MAX_TOUCH_NUM + 1;
        }
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[0],
                              "%s %4s  %s",
                              lvgl_task_app_t.llq_style.font_normal_color, " ", "");
        ...
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[3],
                              "%s %4s  \tSet	:	[%d]",
                              lvgl_task_app_t.llq_style.font_normal_color, " ", enc_t % MAX_TOUCH_NUM + 1);
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[4],
                              "%s %4s  %s",
                              lvgl_task_app_t.llq_style.font_normal_color, " ", "");
    }
    ...
    case 2:
    {
        if (touch_task_ctr.touch_add_num > 0)
        {
            ...
            {
                falsh_store_ctr.llq_store_p.touch_point[touch_task_ctr.touch_add_count] = enc_t % MAX_PASS_NUM;
                lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[1],
                                      "%s %4s  %s",
                                      lvgl_task_app_t.llq_style.font_normal_color, " ", "Press your fingerprint!");
                lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[4],
                                      "%s %4s  \tCurrent    :[%d]!",
                                      lvgl_task_app_t.llq_style.font_normal_color, " ", touch_task_ctr.touch_add_count);
            }
        }
        else
        {
        ...
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[2],
                              "%s %4s  password[%d]",
                              lvgl_task_app_t.llq_style.font_setting_color, " ", enc_t % MAX_PASS_NUM);
    }
    break;
    case 3:
    {
    ...
    case 4:
    {
        ...
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[2],
                              "%s %4s  password[%d]",
                              lvgl_task_app_t.llq_style.font_normal_color, " ", falsh_store_ctr.llq_store_p.touch_point[touch_task_ctr.touch_delect.page_id]);
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[3],
                              "%s %4s  \tpage_id  :\t[%d]",
