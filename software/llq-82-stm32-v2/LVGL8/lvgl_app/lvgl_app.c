 ...
 * @LastEditTime: 2022-02-27 16:00:46
...
void lv_flash_save(void)
{
    static uint8_t time = 0;
    time++;
    lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[0],"%s %4s %s",lvgl_task_app_t.llq_style.font_select_color, LV_SYMBOL_SD_CARD,"llq-82 flash write!!!");
    ...
    if (time % 5 == 0)
    {
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[3], "flash write ");
    }
    else if (time % 5 == 1)
    {
        lv_label_set_text_fmt(lvgl_task_app_t.widget1.lable[3], "flash write ..");
